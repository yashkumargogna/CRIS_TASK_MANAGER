...
{
	var childWinID = winName + removeSpecialChar(strSuffix);
...
function showTrainSpeedBreakUp(idTrainDef,trainNo,routeNo)
{	
	var queryStr = "ReportServlet?action=report&reportAction=Utility&reportType=TrainSpeedBreakUp&subAction=showDetail&idTrainDef="+idTrainDef+"&trainNo="+trainNo+"&routeNo="+routeNo;
	openNewWindow(queryStr,'winTSBU',idTrainDef+routeNo);
}
/* Ends */
...
{	
	var queryStr = "ReportServlet?action=feedback&reportAction=SearchFeedback&feedbackID="+FBID;
...
function showCoachTypePhoto(coachType)
{	
	var queryStr = "CoachTypeDetail.jsp?coachType="+coachType;
	openNewWindow(queryStr,'winCTPH',coachType);
}
/* Ends */
...
{	
	var queryStr = "ReportServlet?reportAction=Utility&reportType=LocoStatus&subAction=show&locoNo="+locoNo;
...
{	
	var queryStr = "http://10.60.200.171/icms_help/zone_help/"+zoneCode+".html";
...
function getViewCount(reportID,reportView)
{
	if(reportView == "M")
	{
		httpRequest = newXMLHttpRequest();
		var queryString = "ReportServlet?action=report&reportAction=Utility&reportType=ReportHitCount&reportID="+reportID;
